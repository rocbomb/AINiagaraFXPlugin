...
```cpp
// NiagaraVariableHelpers.h - C++ 核心 API
// 提供给 Python 调用的函数接口

class UFUNCTION(BlueprintCallable)
class UNiagaraVariableHelpers
{
public:
    // 读取参数（已实现）
    static float GetNiagaraVariableFloat(UNiagaraComponent* NiagaraComp, FString VariableName, FString Namespace);
    static FLinearColor GetNiagaraVariableColor(UNiagaraComponent* NiagaraComp, FString VariableName, FString Namespace);
    static FVector GetNiagaraVariableVec3(UNiagaraComponent* NiagaraComp, FString VariableName, FString Namespace);
    // ... 其他 Get 函数
    
    // 写入参数（部分实现）
    static void SetNiagaraVariableFloat(UNiagaraComponent* NiagaraComp, FString VariableName, float Value, FString Namespace);
    static void SetNiagaraVariableColor(UNiagaraComponent* NiagaraComp, FString VariableName, FLinearColor Value, FString Namespace);
    // ... 需要补全其他 Set 函数
};
```
...
**C++ 层责任**:
- 提供高性能的 Niagara 参数读写
- 批量操作 API（减少 Python 调用开销）
- 参数验证和边界检查
- 暴露为 UFUNCTION，供 Blueprint 和 Python 调用
...
```python
# Content/Python/ui/main_window.py
import unreal
...
class AINiagaraToolWindow:
    def __init__(self):
        ...
        self.ai_assistant = NiagaraAIAssistant()
    ...
    def on_component_selected(self, component):
        ...
        # 获取参数列表（调用 C++ API）
        params = self.param_manager.get_all_parameters(component)
        self.update_parameter_list(params)
    
    def on_ai_input(self, user_text):
        """用户输入 AI 指令"""
        if not self.selected_component:
            unreal.log_warning("请先选择 Niagara 组件")
            return
        
        # 调用 AI 助手
        self.ai_assistant.adjust_parameters(
...
def register_menu():
    ...
    entry.set_tool_tip(unreal.Text("打开 AI Niagara 特效工具"))
...
```python
# Content/Python/ai/openai_client.py
import openai
import json
import unreal
...
class NiagaraAIAssistant:
    def __init__(self):
        api_key = get_api_key()  # 从 UE ProjectSettings 读取
        self.client = openai.OpenAI(api_key=api_key)
    ...
    def adjust_parameters(self, niagara_component, user_input):
        ...
        try:
            ...
            # 4. 应用参数（调用 C++ API）
            self._apply_adjustments(niagara_component, adjustments)
    ...
    def _apply_adjustments(self, component, adjustments):
        """应用参数调整（调用 C++ Set 函数）"""
        for param_name, value in adjustments.get("parameters", {}).items():
            param_type = self._detect_type(value)
            
            if param_type == "float":
                unreal.NiagaraVariableHelpers.set_niagara_variable_float(
                    component, param_name, value, "User"
                )
                ...
                unreal.NiagaraVariableHelpers.set_niagara_variable_color(
                    component, param_name, color, "User"
                )
            # ... 其他类型
```
...
# 1. 获取场景中的 Niagara 组件（C++ 函数）
components = unreal.ExposeNiagaraVariablesEditorBPLibrary.get_all_niagara_components_in_scene()

for comp in components:
    # 2. 读取参数（C++）
    spawn_rate = unreal.NiagaraVariableHelpers.get_niagara_variable_float(
        comp, "SpawnRate", "User"
    )
    
    # 3. Python 中处理
    new_spawn_rate = spawn_rate * 1.5
    
    # 4. 写入参数（C++）
    unreal.NiagaraVariableHelpers.set_niagara_variable_float(
        comp, "SpawnRate", new_spawn_rate, "User"
    )
```
